...
- (BOOL)registerWithPassword:(NSString *)password error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		NSString *username = [myJID domain];
...
- (BOOL)authenticateWithPassword:(NSString *)password error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		else
		{
			...
			NSString *username = [myJID domain];//[myJID user];
			...
			if ([resource length] == 0)
			{
				...
				resource = @"tiebiPushClient";//[self generateUUID];
			}
			
			NSXMLElement *queryElement = [NSXMLElement elementWithName:@"query" xmlns:@"jabber:iq:auth"];//auth
			[queryElement addChild:[NSXMLElement elementWithName:@"username" stringValue:username]];
			...
			NSXMLElement *iqElement = [NSXMLElement elementWithName:@"iq"];
//			[iqElement addAttributeWithName:@"id" stringValue:@"set"];
            [iqElement addAttributeWithName:@"type" stringValue:@"set"];
			[iqElement addChild:queryElement];
...
//<iq xmlns="jabber:client" type="result" to="127.0.0.1/566c71a9"/>
...
- (void)handleStreamFeatures
{
	...
	if (f_bind)
	{
		...
		if ([requestedResource length] > 0)
		{
			...
			XMPPLogSend(@"SEND10: %@", outgoingStr);
			numberOfBytesSent += [outgoingData length];
		...
		else
		{
			...
			XMPPLogSend(@"SEND11: %@", outgoingStr);
			numberOfBytesSent += [outgoingData length];
...
- (void)handleRegistration:(NSXMLElement *)response
{
	...
	{
		// Revert back to connected state (from authenticating state)
		state = STATE_XMPP_AUTH_4;
		
		[multicastDelegate xmppStreamDidRegister:self];
        
        //Kevin add
        NSXMLElement *query = [NSXMLElement elementWithName:@"query" xmlns:@"jabber:iq:auth"];//jabber:iq:auth
        [query addChild:[NSXMLElement elementWithName:@"username" stringValue:[myJID domain]]];
        ...
        XMPPLogSend(@"SEND20: %@", outgoingStr);
        numberOfBytesSent += [outgoingData length];
...
- (void)handleRegistration2:(NSXMLElement *)response{
    if ([[response attributeStringValueForName:@"type"] isEqualToString:@"error"])
	{
		// Revert back to connected state (from authenticating state)
		state = STATE_XMPP_CONNECTED;
  ...
  if ([self supportsDigestMD5Authentication] || isAccessToken)
	{
      ...
      else
      {
        ...
        state = STATE_XMPP_AUTH_2;
			...
			XMPPLogSend(@"SEND12: %@", outgoingStr);
			numberOfBytesSent += [outgoingData length];
...
- (void)handleAuth2:(NSXMLElement *)response
{
	...
	if ([[response name] isEqualToString:@"challenge"])
	{
		...
		else
		{
			...
			XMPPLogSend(@"SEND13: %@", outgoingStr);
			numberOfBytesSent += [outgoingData length];
...
- (void)handleBinding:(NSXMLElement *)response
{
	...
	if (r_jid)
	{
		...
		if (f_session)
		{
			...
			XMPPLogSend(@"SEND14: %@", outgoingStr);
			numberOfBytesSent += [outgoingData length];
	...
	else
	{
		...
		XMPPLogSend(@"SEND15: %@", outgoingStr);
		numberOfBytesSent += [outgoingData length];
...
- (void)tryNextSrvResult
{
	...
	if (!success)
	{
		...
		success = [self connectToHost:[myJID domain] onPort:XMPP_HOSTPORT error:&connectError];
	}
...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag
{
	...
	if (XMPP_LOG_RECV_PRE)
	{
		...
		XMPPLogRecvPre(@"RECV1: %@", dataAsStr);
	}
	...
	dispatch_async(xmppQueue, ^{ @autoreleasepool {
		...
		if ([self isP2P])
		{
			...
			if ([self isP2PRecipient])
			{
				...
				XMPPLogSend(@"SEND16: %@", outgoingStr);
				numberOfBytesSent += [outgoingData length];
		...
		else
		{
			...
			else
			{
				...
				XMPPLogSend(@"SEND17: %@", outgoingStr);
				numberOfBytesSent += [outgoingData length];
...
- (void)xmppParser:(XMPPParser *)sender didReadElement:(NSXMLElement *)element
{
	...
	dispatch_async(xmppQueue, ^{ @autoreleasepool {
		...
		XMPPLogRecvPost(@"RECV3: %@", [element compactXMLString]);
		...
		}else if(state == STATE_XMPP_AUTH_4){
            state = STATE_XMPP_CONNECTED;
            NSString *myPassword = [[NSUserDefaults standardUserDefaults] stringForKey:@"kXMPPmyPassword"];
            [self authenticateWithPassword:myPassword error:nil];
        }
		else
		{
			if ([elementName isEqualToString:@"iq"])
...
//                        [query addChild:[NSXMLElement elementWithName:@"username" stringValue:@""]];
                        ...
                        XMPPLogSend(@"SEND21: %@", outgoingStr);
                        numberOfBytesSent += [outgoingData length];
