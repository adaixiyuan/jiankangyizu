...
- (void)viewDidLoad
{
    ...
    ViWcmCommonMember *user= [NSKeyedUnarchiver unarchiveObjectWithData:userData];
    self.singleInfo=user;
    ...
    [dao searchWhere:@"" orderBy:@"" offset:0 count:PAGESIZE callback:^(NSArray *array){
    ...
    message.textMessage = @"欢迎您，感谢您使用约翰迪尔APP，您有任何问题请留言";
    message.flag=1;
...
//        [NSThread detachNewThreadSelector:@selector(createLocalList:) toTarget:self withObject:localArray2];
    ...
    listView=[[PullingRefreshTableView alloc]initWithFrame:CGRectMake(0, 0, kDeviceWidth, KDeviceHeight-44) pullingDelegate:self];
...
-(void)createLocalList:(NSArray *)_messageArray{
    @autoreleasepool {
        ...
        for (id obj in _messageArray) {
            WcmCmsMemberMessage *_message=obj;
            BOOL isMySend=YES;
            if (_message.addUserId!=userId) {
                isMySend=NO;
            }
                ...
                default:{//普通信息
                    switch (_message.messageTypeId) {
                        ...
                        case 4:{
                            ...
                            BubbleView *chatView = [self bubbleView4:image from:isMySend date:_message.addDate isNew:isRead success:_message.status time:_message.fileTimeLength];
                            [chatView.activityView stopAnimating];
                            chatView.isSuccess=YES;
                            if (_message.status==-1) {
                                chatView.state.hidden=NO;
...
- (void)loadData{
    if (self.refreshing) {
        ...
        FMDatabaseQueue *queue = [[FMDatabaseQueue alloc]initWithPath:[ZUOYLTempHelper getPathForDocuments:@"asiastarbus.db" inDir:@"db"]];
        WcmCmsMemberMessageDAO* dao = [[WcmCmsMemberMessageDAO alloc]initWithDbqueue:queue];
        [dao searchWhere:@"" orderBy:@"" offset:page*PAGESIZE count:PAGESIZE callback:^(NSArray *array){
            self.localArray=(NSMutableArray *) [[array reverseObjectEnumerator] allObjects];
...
//        
//    }
//    listView.frame=CGRectMake(0, 0, kDeviceWidth, [UIScreen mainScreen].bounds.size.height-keyHeight-44);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (newmessage.messageTypeId==7) {
        ...
        titleLabel.text = newmessage.title;
        titleLabel.backgroundColor=[UIColor clearColor];
        ...
        dateLabel.text = newmessage.updateDate;
       ...
       UIImageView *imageViewa=[[UIImageView alloc]initWithFrame:CGRectMake((kDeviceWidth-288-120)/2, 40, 288, 140)];
        ...
        [imageViewa setImageWithURL:[NSURL URLWithString:newmessage.cover] refreshCache:NO placeholderImage:[UIImage imageNamed:@"defaultImage_102x70.png"]];
...
-(void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    if (connection==connection1) {
        ...
        NSLog(@"%@",receiveStr);
        [commonAction showToast:[JsonToModel getMessageFromDictionaryData:receiveData1]];
...
-(NSString *)createProperties:(WcmCmsMemberMessage *)message{
    ...
    [properties appendString:[NSString stringWithFormat:@"&obj.add_user_id=%d",self.singleInfo.identity]];
    [properties appendString:[NSString stringWithFormat:@"&obj.add_user=%@",self.singleInfo.usern]];
